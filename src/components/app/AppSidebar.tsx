    ...
    return (
      <SidebarMenuItem key={item.url}>
        <SidebarMenuButton
          ...
          className="group/nav h-9 rounded-xl px-2.5 text-sidebar-foreground/70 transition-all duration-150
        ...
        >
          <NavLink to={item.url} className="flex items-center gap-2.5">
            <span className="relative shrink-0">
              ...
              />
              {badge > 0 && collapsed && (
                <span className="absolute -top-1.5 -right-1.5 min-w-[14px] h-[14px] px-1 rounded-full grid place-items-center text-[9px] font-bold text-white bg-bad ring-2 ring-sidebar">
                  {badge > 9 ? "9+" : badge}
                </span>
              )}
            </span>
